...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    if (indexPath.section == 0) {
        ...
        if (indexPath.row == 0) {
            ...
            cell.titleLbl.text = localizeString(@"Modo de uso");
            
            if ([RPEntry instance].disableTracking) {
                cell.statusLbl.text = localizeString(@"No habilitado");
            } else {
                cell.statusLbl.text = localizeString(@"Automático");
            }
            
            if ([defaults_object(@"onDemandTracking") boolValue]) {
                cell.statusLbl.text = localizeString(@"Manual");
            }
