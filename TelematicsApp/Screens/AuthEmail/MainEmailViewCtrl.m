...
- (void)viewDidLoad {
    ...
    self.welcomeLbl.text = localizeString(@"Ingresar\n");
    ...
    NSMutableAttributedString *loginText = [[NSMutableAttributedString alloc] initWithString:localizeString(@"INGRESAR")];
    ...
    NSMutableAttributedString *signUpText = [[NSMutableAttributedString alloc] initWithString:localizeString(@"¿No tienes cuenta? Registrar")];
    ...
    self.justUsingLbl.attributedText = [self createJoinLabelImgCentered:localizeString(@"Ingresar via   ") phoneText:localizeString(@"Teléfono")];
            ...
            if (error.code == 17007) { //USER IS EXIST
                ...
                [[FIRAuth auth] signInWithEmail:self.emailField.text
                                     ...
                                     completion:^(FIRAuthDataResult *existUser, NSError *error) {
                                            ...
                                            if (error) {
                                                [GeneralService alert:error.localizedDescription title:@"Mensaje"];
                                                [self hidePreloader];
                                                return;
                                            }
                                            ...
                                            FIRDatabaseQuery *allUserData = [[[GeneralService sharedService].realtimeDatabase child:@"usuarios"] child:existUser.user.uid];
